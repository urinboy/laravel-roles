    ...
    <div>
        <div class="mb-6 w-full">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold mb-2 mt-4 text-gray-900 dark:text-white">{{ __('Edit Role') }}</h2>
                    <p class="text-gray-500 dark:text-gray-300 mb-4">{{ __('Form for editing the role') }}</p>
                </div>
                <div class="mb-6 flex justify-end">
                    <a href="{{ route('roles.index') }}" title="{{ __('Back to Roles List') }}"
        ...
        <div>
            ...
            <form wire:submit.prevent="submit" class="mt-6 space-y-6">
                <flux:input wire:model.live="name" label="{{ __('Role Name') }}" placeholder="{{ __('Enter role name') }}"
                    :error="$errors -> first('name')" />

                <div>
                    <label
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Permissions') }}</label>
                        ...
                        @foreach ($groupedPermissions as $model => $perms)
                            @php
                                $modelPermissions = collect($permissionOrder)
                                    ...
                                    ->toArray();
                                $allChecked =
                                    count($modelPermissions) > 0 &&
                                    count(array_intersect($modelPermissions, $permissions ?? [])) ===
                                        count($modelPermissions);
                            @endphp
                            <div
                                class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow p-4 flex flex-col items-start">
                                <div class="mb-3 w-full flex items-center justify-between">
                                    <span
                                        class="block text-base font-semibold text-gray-900 dark:text-white uppercase tracking-wide">
                                        {{ __(ucfirst($model)) }}
                                    </span>
                                ...
                                <div class="flex flex-col gap-2 w-full">
                                    @foreach ($permissionOrder as $action)
                                        ...
                                        @endphp
                                        <label
                                            class="flex items-center px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                                            <input type="checkbox" wire:model.live="permissions"
                                                value="{{ $permName }}"
                                                class="rounded border-gray-300 text-green-600 shadow-sm focus:ring-green-500"
                                                @if (!in_array($permName, $perms)) disabled @endif>
                                            <span
                                                class="ml-2 text-gray-700 dark:text-gray-300 capitalize {{ !in_array($permName, $perms) ? 'opacity-40 line-through' : '' }}">
                                                {{ __($action) }}
                                            </span>
                                        </label>
                                    @endforeach
