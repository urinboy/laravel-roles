            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3 sm:px-6 text-center">{{ __('Actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($users as $user)
                        <tr
                            ...
                            <td class="px-4 py-3 sm:px-6 font-medium text-gray-900 dark:text-white">{{ $user->id }}
                            </td>
                                ...
                                @if ($user->roles->count())
                                    <div class="flex flex-wrap gap-1">
                                        @foreach ($user->roles as $role)
                                            <span
                                                class="inline-block px-2 py-0.5 rounded bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100 text-xs">
                                                {{ $role->name }}
                                            </span>
                                        @endforeach
                                    </div>
                            ...
                            <td class="px-4 py-3 sm:px-6 flex flex-row gap-2 justify-center">
                                ...
                                @can('user.delete')
                                    ...
                                    <flux:modal name="confirm-user-deletion-{{ $user->id }}" focusable
                                        class="max-w-lg">
                                        <form wire:submit.prevent="delete({{ $user->id }})" class="space-y-6">
                                            <div>
                                                <flux:heading size="lg">
                                                    {{ __('Are you sure you want to delete this user?') }}</flux:heading>
                                                <flux:subheading>
                                                    {{ __('Once this user is deleted, all of their resources and data will be permanently deleted. This action cannot be undone.') }}
                                                </flux:subheading>
                                            </div>
