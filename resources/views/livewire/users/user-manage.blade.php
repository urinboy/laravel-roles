    ...
    <div>
        <div class="mb-6 w-full">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold mb-2 mt-4 text-gray-900 dark:text-white">{{ __('Users') }}</h2>
                    <p class="text-gray-500 dark:text-gray-300 mb-4">{{ __('Manage all your users') }}</p>
                </div>
                @can('user.create')
                    <div class="mb-6 flex justify-end">
                        <button wire:click="openModal('create')" title="{{ __('Add New User') }}"
        ...
        @if (session()->has('success'))
            <div
                ...
                <button type="button" wire:click="$set('showModal', false)"
                    class="text-xl font-bold text-green-400 hover:text-green-700">&times;</button>
            </div>
        @endif
                            ...
                            <td class="px-6 py-3">
                                @if ($user->roles->count())
                                    @foreach ($user->roles as $role)
                                        ...
                                        @if (strtolower($role->name) === 'admin' || strtolower($role->name) === 'administrator') bg-red-500 text-white
                                        @else
